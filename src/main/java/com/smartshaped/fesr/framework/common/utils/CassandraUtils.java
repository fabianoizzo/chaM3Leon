 ...
 * <p>
 * This class provides methods to create a Cassandra session based on the configuration provided by
 * {@link ConfigurationUtils}.
 * <p>
 * This class is a Singleton and can be obtained with the method {@link #getCassandraUtils(ConfigurationUtils)}.
 * <p>
...
public class CassandraUtils {
    ...
    private void createKeyspace(int replicationFactor) throws CassandraException {
        ...
        String query = "CREATE KEYSPACE IF NOT EXISTS ? WITH replication = {'class': 'SimpleStrategy', 'replication_factor': ?};";
        executeQuery(query, keyspace, replicationFactor);
    }

    /**
     * Get a singleton instance of CassandraUtils.
     * <p>
     * This method will create a new instance of CassandraUtils if it does not already exist.
     ...
     */
    public static CassandraUtils getCassandraUtils(ConfigurationUtils configurationUtils) throws ConfigurationException, CassandraException {
     ...
     * <p>
     * This method will read the configuration and create a Cassandra session.
     * If the configuration is invalid, a {@link ConfigurationException} will be thrown.
     * If any error occurs during the creation of the session, a {@link CassandraException} will be thrown.
     * <p>
     ...
     *     <li>{@code cassandra.datacenter}: the Cassandra data center to connect to</li>
     * </ul>
     *
     ...
     * <p>
     * The method will return a {@link ResultSet} containing the result of the query.
     * <p>
     * If any error occurs during the execution, a {@link CassandraException} will be thrown.
     *
     ...
     * <p>
     * This method will generate the query string based on the provided column-value map.
     * <p>
     * The method will return a {@link ResultSet} containing the result of the query.
     * <p>
     * If any error occurs during the execution, a {@link CassandraException} will be thrown.
    ...
    public ResultSet executeInsert(String tableName, Map<String, Object> columnValueMap) throws CassandraException {
        ...
        String query = "INSERT INTO " + keyspace + "." + tableName + " (" + columns.toString() + ") VALUES (" + valuesPlaceholders.toString() + ")";
     ...
     * <p>
     * The method will generate the query string based on the provided column-value map.
     * <p>
     * The method will return a {@link ResultSet} containing the result of the query.
     * <p>
     * If any error occurs during the execution, a {@link CassandraException} will be thrown.
     ...
     * @throws CassandraException if any error occurs during the execution
     */
    public ResultSet executeUpdate(String tableName, Map<String, Object> values, Optional<String> whereClause) throws CassandraException {
        StringBuilder queryBuilder = new StringBuilder("UPDATE ").append(keyspace).append(".").append(tableName).append(" SET ");
    ...
    /**
     * Validates a TableModel by checking if the table exists in Cassandra.
    ...
    private void createTable(TableModel tableModel) throws CassandraException {
        ...
        String query = tableModel.getCreationQuery();
        try {
            executeQuery(query, keyspace);
     ...
     * <p>
     * This method will check if the TableModel specifies a primary key. If not,
     * it will generate a UUID as a primary key and add it to the DataFrame.
     * Then, it will save the DataFrame to the Cassandra table using the
     ...
     * @param tableModel the TableModel from which to get the table name and
    ...
    public void saveDF(Dataset<Row> df, TableModel tableModel) throws CassandraException {
        ...
        // Add the generated UUID as a column if no primary key was specified
        if (generateUuid) {
